...
Options:
  ...
  --repo <name>           Repository name (default: baton)
...
gh_content_sha_with_retry() {
  ...
  while true; do
    err_file="$(mktemp -t baton-publish-sha.XXXXXX)"
...
gh_put_content_with_retry() {
  ...
  while true; do
    err_file="$(mktemp -t baton-publish-put.XXXXXX)"
...
gh_repo_accessible_with_retry() {
  ...
  while true; do
    err_file="$(mktemp -t baton-publish-repo.XXXXXX)"
...
for file in "${FILES[@]}"; do
  ...
  content_file="$(mktemp -t baton-publish-content.XXXXXX)"
  base64 -i "$file" | tr -d '\n' >"$content_file"
  payload="$(mktemp -t baton-publish-payload.XXXXXX)"
